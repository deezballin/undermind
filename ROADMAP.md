...
- **Ran on primary, zero model load.** Result: fan 972 → prune 162 (greeting) → re-widen 324 (emotional arrives) → SELECT `emotional:acknowledge` (score 3.0, the 3 emotional words). **No generation, pure selection.** Proves the mechanism honestly.
...
5. ⏳ Sandbox: dream loop → feelings/soul skills.
6. ⏳ Verify the FEEL: does it interrupt mid-type? anticipate correctly? If yes → promote. If noise → sandbox stays.

## Privacy
- Undermind ships as a **framework** (mechanism only). No operator persona, no
  memory dumps, no personal data in the repo. Keep it that way.
